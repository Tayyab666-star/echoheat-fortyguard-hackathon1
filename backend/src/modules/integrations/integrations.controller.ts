...
import { fortygardClient, isError } from "./services/fortygard.service";
import { samsaraService } from "./services/samsara.service";
import { procoreService } from "./services/procore.service";
import {
  fortygardSnapshotSchema,
  fortygardCorridorSchema,
  samsaraConnectSchema,
  procoreConnectSchema,
} from "./validators";
...
export async function getFortygardSnapshot(req: Request, res: Response): Promise<void> {
  try {
    const { lat, lng, radiusMeters }: any = validate(fortygardSnapshotSchema, req.query);
...
export async function getFortygardCorridor(req: Request, res: Response): Promise<void> {
  try {
    const { waypoints }: any = validate(fortygardCorridorSchema, req.body);
...
export async function connectSamsara(req: Request, res: Response): Promise<void> {
  try {
    const data: any = validate(samsaraConnectSchema, req.body);
...
export async function handleProcoreCallback(req: Request, res: Response): Promise<void> {
  try {
    const { code }: any = validate(procoreConnectSchema, req.query);
    const orgId = (req as any).user?.organization as string;
